...
def empty_input_loss_func_wrapper(loss_func):
    def wrapped_loss_func(input, target, *, reduction="mean", **kwargs):
        ...
        """
        if target.numel() == 0 and reduction in ("mean", "sum"):
            return input.sum() * 0.0  # connect the gradient
        elif torch.all(target.eq(-100)) and reduction in ("mean", "sum"):
            return input.sum() * 0.0  # connect the gradient
...
def get_1d_sincos_pos_embed(embed_dim, length, cls_token=False):
    ...
    emb = get_1d_sincos_pos_embed_from_grid(embed_dim, indices)
...
def get_length_grouped_indices(indices, lengths, batch_size, world_size):
