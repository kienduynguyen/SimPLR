...
__device__ scalar_t fast_instance_attn_im2col_bilinear(
...
template <typename scalar_t>
__device__ void fast_instance_attn_col2im_bilinear(
    const scalar_t* &bottom_data,
    const int &height,
    const int &width,
    const int &nheads,
...
template <typename scalar_t>
__device__ void fast_instance_attn_col2im_bilinear_gm(
    const scalar_t* &bottom_data,
    const int &height,
    const int &width,
    const int &nheads,
...
template <typename scalar_t>
__global__ void fast_instance_attn_im2col_gpu_kernel(
    const int n,
...
)
{
    CUDA_KERNEL_LOOP(index, n)
    {
        ...
        _temp /= num_point;
        int data_index = _temp * channels + c_col;
        ...
        scalar_t* data_col_ptr = data_col + data_index;
        const int data_weight_ptr_init_offset = sampling_index * num_levels * num_point;
        const int data_loc_w_ptr_init_offset = data_weight_ptr_init_offset << 1;
        ...
        for (int l_col = 0; l_col < num_levels; ++l_col)
        {
            ...
            const int n_point_offset = l_col * num_point + p_col;
            const int data_loc_w_ptr = data_loc_w_ptr_init_offset + (n_point_offset << 1);
            const scalar_t loc_w = data_sampling_loc[data_loc_w_ptr];
            const scalar_t loc_h = data_sampling_loc[data_loc_w_ptr + 1];

            const int data_weight_ptr = data_weight_ptr_init_offset + n_point_offset;
            ...
            {
                const scalar_t val = fast_instance_attn_im2col_bilinear(
                    data_value_ptr,
                    spatial_h,
                    spatial_w,
                    num_heads,
                ...
                atomicAdd(data_col_ptr, val * spatial_weight);
...
__global__ void fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1(
    const int n,
...
)
{
    CUDA_KERNEL_LOOP(index, n)
    {
        ...
        const scalar_t top_grad = grad_col[data_index];

        int data_weight_ptr_init_offset = sampling_index * num_levels * num_point;
        int data_loc_w_ptr_init_offset = data_weight_ptr_init_offset << 1;
        const int grad_sampling_ptr = data_weight_ptr_init_offset + p_col;
        ...
        for (int l_col = 0; l_col < num_levels; ++l_col)
        {
            ...
            *(cache_grad_level_attn_weight + threadIdx.x) = 0;
            ...
            if (h_im > -1 && w_im > -1 && h_im < spatial_h && w_im < spatial_w)
            {
                fast_instance_attn_col2im_bilinear(
                    data_value_ptr,
                    spatial_h,
                    spatial_w,
                    num_heads,
            ...
            grad_spatial_attn_weight += num_point;
            grad_level_attn_weight += num_point;
            grad_sampling_loc += num_point << 1;
        }
    }
}
...
template <typename scalar_t, unsigned int blockSize>
__global__ void fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2(
    const int n,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
)
{
    CUDA_KERNEL_LOOP(index, n)
    {
        __shared__ scalar_t cache_grad_sampling_loc[blockSize << 1];
        __shared__ scalar_t cache_grad_spatial_attn_weight[blockSize];
        __shared__ scalar_t cache_grad_level_attn_weight[blockSize];
...
template <typename scalar_t>
__global__ void fast_instance_attn_col2im_gpu_kernel_shm_reduce_v1(
    const int n,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
template <typename scalar_t>
__global__ void fast_instance_attn_col2im_gpu_kernel_shm_reduce_v2(
    const int n,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
template <typename scalar_t>
__global__ void fast_instance_attn_col2im_gpu_kernel_shm_reduce_v2_multi_blocks(
    const int n,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
template <typename scalar_t>
__global__ void fast_instance_attn_col2im_gpu_kernel_gm(
    const int n,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
)
{
    CUDA_KERNEL_LOOP(index, n)
    {
        ...
        for (int l_col = 0; l_col < num_levels; ++l_col)
        {
            ...
            if (h_im > -1 && w_im > -1 && h_im < spatial_h && w_im < spatial_w)
            {
                fast_instance_attn_col2im_bilinear_gm(
                    data_value_ptr,
                    spatial_h,
                    spatial_w,
                    num_heads,
...
template <typename scalar_t>
void fast_instance_attn_im2col_cuda(
    cudaStream_t stream,
    const scalar_t* data_value,
    const int64_t* data_spatial_shapes,
    const int64_t* data_level_start_index,
...
{
    const int num_kernels = batch_size * num_query * num_heads * num_point * channels;
    const int num_actual_kernels = batch_size * num_query * num_heads * num_point * channels;
    const int num_threads = CUDA_NUM_THREADS;

    fast_instance_attn_im2col_gpu_kernel<scalar_t>
    ...
    if (err != cudaSuccess)
    {
        printf("error in fast_instance_attn_im2col_cuda: %s\n", cudaGetErrorString(err));
    }
}


template <typename scalar_t>
void fast_instance_attn_col2im_cuda(
    cudaStream_t stream,
    const scalar_t* grad_col,
    const scalar_t* grad_mask_col,
    const scalar_t* data_value,
...
)
{
    ...
    if (channels > 1024)
    {
        if ((channels & 1023) == 0)
        {
            fast_instance_attn_col2im_gpu_kernel_shm_reduce_v2_multi_blocks<scalar_t>
        ...
        {
            fast_instance_attn_col2im_gpu_kernel_gm<scalar_t>
                <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                    num_kernels,
                    grad_col,
                    grad_mask_col,
    ...
    else
    {
        ...
        {
            case 1:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 1>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 2:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 2>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 4:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 4>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 8:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 8>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 16:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 16>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 32:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1<scalar_t, 32>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 64:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2<scalar_t, 64>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 128:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2<scalar_t, 128>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 256:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2<scalar_t, 256>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 512:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2<scalar_t, 512>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
                ...
                break;
            case 1024:
                fast_instance_attn_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2<scalar_t, 1024>
                    <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 0, stream>>>(
                        num_kernels,
                        grad_col,
                        grad_mask_col,
            ...
            default:
                if (channels < 64)
                {
                    fast_instance_attn_col2im_gpu_kernel_shm_reduce_v1<scalar_t>
                        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 
                        num_threads * 4 * sizeof(scalar_t), stream>>>(
                            num_kernels,
                            grad_col,
                ...
                {
                    fast_instance_attn_col2im_gpu_kernel_shm_reduce_v2<scalar_t>
                        <<<GET_BLOCKS(num_actual_kernels, num_threads), num_threads, 
                        num_threads * 4 * sizeof(scalar_t), stream>>>(
                            num_kernels,
                            grad_col,
    ...
    if (err != cudaSuccess)
    {
        printf("error in fast_instance_attn_col2im_cuda: %s\n", cudaGetErrorString(err));
    }
}
