#include "fast_box_attn_kernel.cuh"
...
at::Tensor fast_box_attn_cuda_forward(
...
)
{
    ...
    for (int n = 0; n < batch / im2col_step_; ++n) {
        ...
        AT_DISPATCH_FLOATING_TYPES(value.scalar_type(), "fast_box_attn_forward_cuda", ( [&] {
            fast_box_attn_im2col_cuda(
...
std::vector<at::Tensor> fast_box_attn_cuda_backward(
    const at::Tensor &value,
    const at::Tensor &spatial_shapes,
    const at::Tensor &level_start_index,
    const at::Tensor &sampling_loc,
...
)
{
    ...
    for (int n = 0; n < batch / im2col_step_; ++n) {
        ...
        AT_DISPATCH_FLOATING_TYPES(value.scalar_type(), "fast_box_attn_backward_cuda", ( [&] {
            fast_box_attn_col2im_cuda(
                at::cuda::getCurrentCUDAStream(),
