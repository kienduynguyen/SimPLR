...
from e2edet.utils.distributed import shared_random_seed
...
class DistributedSamplerv2(Sampler[T_co]):
    def __init__(
        self,
        dataset: Dataset,
        num_replicas: Optional[int] = None,
        ...
        self.total_size = self.num_samples * self.num_replicas * self.num_epoch
    ...
    def __iter__(self) -> Iterator[T_co]:
        ...
        for i in range(self.num_epoch):
            if self.shuffle:
                # deterministically shuffle based on epoch and seed
                g = torch.Generator()
                g.manual_seed(self.seed + self.epoch * self.num_epoch + i)
                indices = torch.randperm(len(self.dataset), generator=g).tolist()  # type: ignore[arg-type]
            else:
                indices = list(range(len(self.dataset)))  # type: ignore[arg-type]
            total_indices.extend(indices)

        total_indices = total_indices[: self.total_size]
        assert (
            len(total_indices) == self.total_size
        ), f"len={len(total_indices)}, total_size={self.total_size}"

        # subsample
        total_indices = total_indices[self.rank : self.total_size : self.num_replicas]
        assert len(total_indices) == (self.num_samples * self.num_epoch)

        return iter(total_indices)

    def __len__(self) -> int:
        return self.num_samples * self.num_epoch
