...
model_urls = {
    "swin_base_in21k_384": "https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_base_patch4_window12_384_22k.pth",
    "swin_large_in21k_384": "https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_large_patch4_window12_384_22k.pth",
}
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        for i_layer in range(self.num_layers):
            use_checkpoint = i_layer >= (self.num_layers - checkpoint_layer)
            layer = BasicLayer(
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                outs["layer{}".format(i + 1)] = out
                # print(f"layer{i+1}:", out.shape)
                # synchronize()
...
class SwinBackBone(SwinTransformer):
    def __init__(self, **kwargs):
        position_encoding = kwargs.pop("position_encoding", None)
        return_interm_layers = kwargs.pop("return_interm_layers", None)
        ref_size = kwargs.pop("ref_size", None)
        hidden_dim = kwargs.pop("hidden_dim", None)
        pos_mode = kwargs.pop("pos_mode", "sum")

        super(SwinBackBone, self).__init__(**kwargs)

        if return_interm_layers is not None:
            num_channels = [
                self.num_features[int(layer[-1]) - 1]
                for layer in sorted(return_interm_layers)
            ]
        else:
            num_channels = [self.num_features[-1]]

        self.position_encoding = None
        if position_encoding is not None:
            self.position_encoding = build_position_encoding(
                position_encoding, hidden_dim, mode=pos_mode
            )
        ...
        for layer in self.return_layers:
            if layer in y:
                ...
                if self.position_encoding is not None:
                    pos.append(self.position_encoding(x, x_mask, self.ref_size))
                else:
                    pos.append(None)
...
def _load_state_dict(arch, pretrained, pretrained_path, model_data_dir):
    assert not (
        pretrained is False and pretrained_path is not None
...
def build_swin(config):
    ...
    with omegaconf.open_dict(params):
        ...
        elif arch == "swin_large_in21k_384":
            embed_dim = 192
