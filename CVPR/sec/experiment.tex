...
\boldparagraph{A single-scale detector is non-trivial.} In \cref{fig:single_scale}, we explore the use of single-scale input feature by simply projecting the last feature map ($\frac{1}{16}$ scale) of the ViT backbone to the detection head. 
...
\boldparagraph{\ours is an effective single-scale detector.} In \cref{tab:compare}, we show the comparison between \ours and recent object detectors using the plain backbone ViT. We also implement a strong baseline of DeformableDETR~\cite{zhu2021deformable} with SimpleFPN under our end-to-end framework for better comparison. We report ViTDet with the setting in \cite{li2022vitdet}; and other methods using the $5\times$ schedule. The plain detector, \ours, with both scale-aware box-attention (SAB) and scale-aware deformable attention (SAD) removes the need for multi-scale adaptation of the ViT.
...
% \cref{tab:compare} indicates that \ours with single-scale input outperforms ViTDet and reaches competitive performance compared to BoxeR. This is different from recent object detectors with transformers \citep{zhu2021deformable,cheng2022mask2former} in which multi-scale feature maps are needed to achieve strong performance for both object detection and segmentation.
